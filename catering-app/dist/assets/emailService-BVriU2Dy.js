import{c as p,s as m}from"./index-B7RNjdIi.js";const u=p("Send",[["path",{d:"m22 2-7 20-4-9-9-4Z",key:"1q3vgg"}],["path",{d:"M22 2 11 13",key:"nzbqef"}]]),r=async({to:e,subject:a,html:t})=>{if(!e||!a||!t)return{success:!1,error:"Missing required fields"};try{const{data:{session:s}}=await m.auth.getSession(),i=await fetch("https://uitplgqukaxrribgrpvv.supabase.co/functions/v1/send-email",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${(s==null?void 0:s.access_token)||""}`},body:JSON.stringify({to:e,subject:a,html:t})}),c=await i.json();return i.ok?{success:!0,data:c}:(console.error("Email error:",c),{success:!1,error:c.error||"Failed to send email"})}catch(s){return console.error("Email error:",s),{success:!1,error:s.message}}},d=e=>`₱${(e==null?void 0:e.toLocaleString())||0}`,n=e=>e?new Date(e+"T00:00:00").toLocaleDateString("en-PH",{weekday:"long",year:"numeric",month:"long",day:"numeric",timeZone:"Asia/Manila"}):"",o=e=>{if(!e)return"";const[a,t]=e.split(":"),s=parseInt(a),i=s>=12?"PM":"AM";return`${s>12?s-12:s===0?12:s}:${t} ${i}`},l={newBookingCustomer:e=>{var a,t;return{subject:`Booking Confirmed - Red Carpet Catering #${(a=e.id)==null?void 0:a.slice(0,8)}`,html:`
      <!DOCTYPE html>
      <html>
      <head>
        <style>
